...
def downsample_bam(
    ...
    cmd = [samtools, "view", "-b", "-f", "2", "-F", "256", "-F", "2048"]
    if mapq is not None:
        cmd += ["-q", str(mapq)]
    cmd += ["-@", str(threads), "-s", f"{seed}.{frac_int:06d}", str(in_bam), "-o", str(out_bam)]
    sh(cmd)
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        ...
        for i, b in enumerate(bg_list):
            ...
            frac = quota / bt if bt > 0 else 0.0
        ...
        sh([samtools, "sort", "-@", str(args.threads), "-o", str(out_bam), str(final_tmp)])
