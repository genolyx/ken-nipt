...
# ==============================================================
CHROMOSOME_MAPPING = {
    "Chromosome 1": "chr1", "Chromosome 2": "chr2", "Chromosome 3": "chr3", 
    "Chromosome 4": "chr4", "Chromosome 5": "chr5", "Chromosome 6": "chr6",
    "Chromosome 7": "chr7", "Chromosome 8": "chr8", "Chromosome 9": "chr9",
    "Chromosome 10": "chr10", "Chromosome 11": "chr11", "Chromosome 12": "chr12",
    "Chromosome 13": "chr13", "Chromosome 14": "chr14", "Chromosome 15": "chr15",
    "Chromosome 16": "chr16", "Chromosome 17": "chr17", "Chromosome 18": "chr18",
    "Chromosome 19": "chr19", "Chromosome 20": "chr20", "Chromosome 21": "chr21",
    "Chromosome 22": "chr22", "Chromosome X": "chrX", "Chromosome Y": "chrY"
}
        ...
        if df is not None and len(df.columns) >= 2:
            ...
            for i, detection in enumerate(detection_col):
                if detection != "Not Detected":
                    disease = disease_col.iloc[i] if i < len(disease_col) else f"Disease_{i}"
    ...
    try:
        ...
        try:
            ...
            for idx, row in ff_df.iterrows():
                ff_type = str(row.iloc[0])  # 첫 번째 컬럼 (Unnamed: 0 또는 첫 번째 컬럼)
        ...
        try:
            ...
            for idx, row in gender_df.iterrows():
                ...
                if gd_type == 'gd_2' and len(row) >= 3:  # gd_2 행이고 gender 컬럼이 있는 경우
...
def determine_final_trisomy_result(trisomy_details_chromosomes):
    ...
    chr_to_trisomy = {
        "chr21": "T21", "chr18": "T18", "chr13": "T13",
        "chr9": "T9", "chr16": "T16", "chr22": "T22"
    }
...
def build_trisomy_results(analysis_dir, sample_name, fetus_gender, age, common_data_dir):
    ...
    other_trisomy_order = [
        "Trisomy1", "Trisomy2", "Trisomy3", "Trisomy4", "Trisomy5", "Trisomy6",
        "Trisomy7", "Trisomy8", "Trisomy10", "Trisomy11", "Trisomy12",
        "Trisomy14", "Trisomy15", "Trisomy17", "Trisomy19", "Trisomy20"
    ]

    # Other chromosome mapping
    other_chr_to_trisomy = {
        "chr1": "Trisomy1", "chr2": "Trisomy2", "chr3": "Trisomy3", "chr4": "Trisomy4",
        "chr5": "Trisomy5", "chr6": "Trisomy6", "chr7": "Trisomy7", "chr8": "Trisomy8",
        "chr10": "Trisomy10", "chr11": "Trisomy11", "chr12": "Trisomy12",
        "chr14": "Trisomy14", "chr15": "Trisomy15", "chr17": "Trisomy17",
        "chr19": "Trisomy19", "chr20": "Trisomy20"
    }
    ...
    for chr_name, trisomy_name in other_chr_to_trisomy.items():
        ...
        if chr_name in orig_df['chr'].values:
            ...
            if "Not Detected" not in orig_row['result']:
                other_detected = True
                logger.info(f"Other trisomy detected in orig: {chr_name} - {orig_row['result']}")
                break
        ...
        if chr_name in fetus_df['chr'].values:
            ...
            if "Not Detected" not in fetus_row['result']:
                other_detected = True
                logger.info(f"Other trisomy detected in fetus: {chr_name} - {fetus_row['result']}")
                break
...
def read_chromosome_analysis_from_ezd_detailed(file_path):
    ...
    logger.info(f"EZD file read successfully. Shape: {df.shape}, Columns: {list(df.columns)}")
    ...
    for _, row in df.iterrows():
        ...
        logger.debug(f"Processing {chr_name} -> {chr_display_name}: {detection_result} -> {detection}")
    ...
    try:
        ...
        for _, row in threshold_df.iterrows():
            ...
            if chr_name in ['chrX', 'chrY']:
                ...
                uar_min = str(row.get('UAR_min', '')) if pd.notna(row.get('UAR_min', '')) else ''
                uar_max = str(row.get('UAR_max', '')) if pd.notna(row.get('UAR_max', '')) else ''
                z_min = str(row.get('Z_min', '')) if pd.notna(row.get('Z_min', '')) else ''
                z_max = str(row.get('Z_max', '')) if pd.notna(row.get('Z_max', '')) else ''
                
                uar_threshold = f"L < {uar_min}~{uar_max} < H" if uar_min and uar_max else "L < 8.48~8.53 < H"
                z_threshold = f"L < {z_min}~{z_max} < H" if z_min and z_max else "L < 1.7~2.7 < H"
            ...
            for _, row in wc_df.iterrows():
                ...
                for md_key, md_data in results.items():
                    if md_data['disease_name'] == disease:
                        ...
                        results[md_key]["detected_region_link"]["WC"] = f"https://deciphergenomics.org/browser#q/grch37:{detected_region}"
                        break
        ...
        if all_detections:
            ...
            for i, disease in enumerate(sorted_diseases, 1):
                ...
                results[md_key] = {
                    ...
                    "target_region": detection_data["WC"]['detected_region'] if detection_data["WC"] else detection_data["WCX"]['detected_region'],
                    "detection": {"WC": None, "WCX": None},
                    "detected_region": {"WC": None, "WCX": None},
                    "length": {"WC": None, "WCX": None},
                    "z_score": {"WC": None, "WCX": None},
                ...
                if detection_data["WC"]:
                    ...
                    results[md_key]["detection"]["WC"] = "High Risk"
                    results[md_key]["detected_region"]["WC"] = wc_data['detected_region']
                    results[md_key]["length"]["WC"] = wc_data['length']
                    results[md_key]["z_score"]["WC"] = wc_data['z_score']
                    results[md_key]["detected_region_link"]["WC"] = f"https://deciphergenomics.org/browser#q/grch37:{wc_data['detected_region']}"

                # WCX 데이터 채우기
                if detection_data["WCX"]:
                    wcx_data = detection_data["WCX"]
                    results[md_key]["detection"]["WCX"] = "High Risk"
                    results[md_key]["detected_region"]["WCX"] = wcx_data['detected_region']
...
def build_nipt_json(analysis_dir, output_dir, ref_dir, sample_name, fetus_gender, age, version, target_bed_dir, config):
        ...
        if threshold_data:
            for chr_name, chr_data in result_table.items():
                if chr_name in threshold_data:
                    chr_data["Z-score threshold"] = threshold_data[chr_name]['z_threshold']  # None → "L<2.18~2.7<H"
                    chr_data["UAR threshold"] = threshold_data[chr_name]['uar_threshold']    # None → "L<8.45~8.50<H"
        
        output[APPID]["trisomy_details"][group] = {**files, "result_table": result_table}
    ...
    try:
        ...
        if not os.path.exists(qc_file):
            ...
            # 기본 QC 데이터 생성
            sequencing_metrics = {
                "total_reads": {"value": 0, "status": "UNKNOWN", "unit": "reads", "threshold": ">10M"},
                "mapped_reads": {"value": 0, "status": "UNKNOWN", "unit": "reads", "threshold": ">85%"},
                "mapping_rate": {"value": 0, "status": "UNKNOWN", "unit": "%", "threshold": ">85%"},
                "duplicated_reads": {"value": 0, "status": "UNKNOWN", "unit": "reads", "threshold": "<30%"},
                "duplication_rate": {"value": 0, "status": "UNKNOWN", "unit": "%", "threshold": "<40%"},
                "mean_mapping_quality": {"value": 0, "status": "UNKNOWN", "unit": "score", "threshold": ">20"},
                "mean_coverage": {"value": 0, "status": "UNKNOWN", "unit": "X", "threshold": ">0.1X"},
                "gc_content": {"value": 0, "status": "UNKNOWN", "unit": "%", "threshold": "33-55%"}
            }
        else:
...
if __name__ == '__main__':
    ...
    pgroup.add_argument('-analysis_dir', dest='analysis_dir', type=lambda x:file_check(parser, x), help='Analysis directory')
    ...
    pgroup.add_argument('-output_dir', dest='output_dir', type=lambda x:file_check(parser, x), help='Output directory')
    pgroup.add_argument('-target_bed_dir', dest='target_bed_dir', type=lambda x:file_check(parser, x), help='Target bed directory')
