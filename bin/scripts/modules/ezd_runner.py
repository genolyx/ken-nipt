...
def run_decision(ezd_df, threshold_path, config_type='orig'):
    ...
    for i in range(22):
        ...
        if chr_row.empty or row.empty:
            ...
            if z_value >= z_max and uar_value >= ur_max:
                decision = "Detected"
            elif z_min <= z_value and ur_min <= uar_value:
                decision = "Suspected"
            else:
                decision = "Not Detected"
    ...
    logger.info(f"Trisomy 결과 ({config_type}): Detected={detected_count}, Suspected={suspected_count}")
...
def add_sca_to_decision_df(decision_df, sca_results, config_type='orig'):
    ...
    try:
        ...
        if 'female' in sca_results:
            ...
            logger.info(f"Female SCA 추가: chrX, {female_data['detection']} ({config_type})")
        ...
        if 'male' in sca_results:
            ...
            logger.info(f"Male SCA 추가: chrY, {male_data['detection']} ({config_type})")
...
def run_sca_detection(ezd_df, config, config_type='orig'):
    ...
    try:
        ...
        logger.info(f"SCA Detection data extraction:")
        ...
        if test_ur_x is not None and test_z_score is not None:
            female_results = sca_detector.compare_configs(test_ur_x, z_score=test_z_score, gender='female')
            female_detection = female_results.get(config_type, {}).get('result', 'Not Detected')
        ...
        if test_ur_x is not None and test_ur_y is not None:
            male_results = sca_detector.compare_configs(test_ur_x, ur_y=test_ur_y, gender='male')
            male_detection = male_results.get(config_type, {}).get('result', 'Not Detected')
...
def run_ezd_group(sample_name: str, group: str, wig_path: str, labcode: str, analysis_dir: str, data_dir: str, config):
    ...
    threshold_path = join(data_dir, 'refs', labcode, 'EZD', group, f'{group}_thresholds_new.tsv')
...
def save_results_to_tsv(results, output_dir):
    ...
    with open(output_file, 'w') as f:
        # 헤더 작성
        f.write("Category\tChromosome\tMedian_Value\tFirst_Zscore\tSecond_Zscore\tUAR_Percent\n")
        ...
        for i in range(22):
            ...
            f.write(f"Autosome\t{chr_name}\t{median_val:.6f}\t{first_z:.6f}\t{second_z:.6f}\t{ur_pct:.4f}\n")

        # X 염색체 데이터
        f.write(f"Sex_Chromosome\tchrX\t-\t{results['first_zscore_X']:.6f}\t{results['second_zscore_X']:.6f}\t{results['UAR_X']:.4f}\n")
...
def add_sca_lines_to_chrx_plot(ax, config_type='orig'):
    ...
    if sca_detector is None:
        ...
        ax.axhline(y=-6, color='gray', linestyle='--', alpha=0.8, linewidth=1)  # XO threshold
        ax.axhline(y=4.5, color='gray', linestyle='--', alpha=0.8, linewidth=1)  # XXX threshold
...
def plot_chr_scatter_grid(chr_table_dir, sample_name, threshold_path, output_path, group, test_data_dir=None, dpi=200):
    ...
    try:
        logger.info(f"Creating scatter grid plot...")
        ...
        if test_data_dir and os.path.exists(test_data_dir):
            ...
            group_name = os.path.basename(threshold_path).replace('_thresholds_new.tsv', '')
            ...
            logger.info(f"    No test data directory provided or not found")
        ...
        chr_files = [f for f in files_in_dir if f.startswith('chr') and f.endswith('.txt')]
        ...
        for i in range(22):
            ...
            try:
                ...
                df_N = df[df['type'] == 'N']
                if not df_N.empty:
                    ax.scatter(df_N['UAR'], df_N['Z'], 
                             color='black', s=8, alpha=0.6, 
                ...
                df_P = df[df['type'] == 'P']
                if not df_P.empty:
                    ax.scatter(df_P['UAR'], df_P['Z'], 
                             color='green', s=8, alpha=0.6,
                ...
                if chr_name in test_sample_data:
                    ...
                    try:
                        ...
                        ax.scatter(test_ur_num, test_z_num, 
                                 color='red', edgecolor='white', s=50, linewidths=2,
                        ...
                        logger.info(f"        Test sample data invalid: UAR={test_ur}, Z={test_z}")
                else:
                ...
                if not legend_added and (not df_N.empty or not df_P.empty or test_ur is not None):
                ...
                if not df.empty:
                    ...
                    if len(ur_values) > 0 and len(z_values) > 0:
                        ...
                        ax.set_xlim(ur_values.min() - ur_margin, ur_values.max() + ur_margin)
                        ax.set_ylim(z_values.min() - z_margin, z_values.max() + z_margin)
        ...
        try:
            ...
            if os.path.exists(female_file):
                ...
                if not xx_samples.empty:
                    ax.scatter(xx_samples['UAR'], xx_samples['Z'], 
                ...
                if not xxx.empty:
                    ax.scatter(xxx['UAR'], xxx['Z'], 
                             color='orange', s=8, alpha=0.5,
                             facecolors='none', edgecolors='orange', linewidths=0.5,
                             label='XXX')
                             #marker='^', label='XXX')
                ...
                if not xo.empty:
                    ax.scatter(xo['UAR'], xo['Z'], 
                             color='coral', s=8, alpha=0.5,
                             facecolors='none', edgecolors='coral', linewidths=0.5,
                             label='XO')
                             #marker='^', label='XO')
                ...
                if 'chrX' in test_sample_data:
                    ...
                    try:
                        ...
                        ax.scatter(test_ur_x_num, test_z_x_num, 
                                 color='red', edgecolor='white', s=50, linewidths=2,
                                 zorder=10)
                    except (ValueError, TypeError):
                        pass
        ...
        try:
            ...
            if os.path.exists(male_file):
                ...
                if not xy_samples.empty:
                    ax.scatter(xy_samples['UAR.X'], xy_samples['UAR.Y'], 
                             color='black', s=8, alpha=0.5,
                             facecolors='none', edgecolors='black', linewidths=0.5,
                             label='XY')
                ...
                if not xxy.empty:
                    ax.scatter(xxy['UAR.X'], xxy['UAR.Y'], 
                             color='orange', s=8, alpha=0.5,
                             facecolors='none', edgecolors='orange', linewidths=0.5,
                             label='XXY')
                             #marker='^', label='XXY')
                ...
                if not xyy.empty:
                    ax.scatter(xyy['UAR.X'], xyy['UAR.Y'], 
                             color='green', s=8, alpha=0.5,
                             facecolors='none', edgecolors='green', linewidths=0.5,
                             label='XYY')
                             #marker='^', label='XYY')
                ...
                if not xxyy.empty:
                    ax.scatter(xxyy['UAR.X'], xxyy['UAR.Y'], 
                             color='purple', s=8, alpha=0.5,
                             facecolors='none', edgecolors='purple', linewidths=0.5,
                             label='XXYY')
                             #marker='P', label='XXYY')
                ...
                if test_ur_x is not None and test_ur_y is not None:
                    try:
                        ...
                        ax.scatter(test_ur_x_num, test_ur_y_num, 
                                 color='red', edgecolor='darkred', s=50, linewidths=2,
                                 zorder=10)
                    except (ValueError, TypeError):
                        pass
            ...
            ax.text(0.5, 0.5, 'SCA (Male)\nError', 
                   ha='center', va='center', transform=ax.transAxes,
                   fontsize=10, color='red')
            ax.set_title('SCA (Male)', fontsize=12, fontweight='bold')
        
        # 전체 제목 설정
        group_name = os.path.basename(threshold_path).replace('_thresholds.tsv', '').upper()
        if not group_name:
            group_name = "EZD"
            
        plt.suptitle(f'Chromosome UAR vs Z-score Analysis\n{sample_name} - {group_name})', 
        ...
        try:
            ...
            ax.text(0.5, 0.5, f'Error creating plot:\n{str(e)}', 
                   ha='center', va='center', transform=ax.transAxes, 
                   fontsize=12, color='red', bbox=dict(boxstyle="round,pad=0.3", facecolor="lightgray"))
            ...
            logger.info(f"  Failed to save error plot")
