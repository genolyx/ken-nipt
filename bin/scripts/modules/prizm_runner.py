...
def setup_logging(debug: bool = False) -> logging.Logger:
    ...
    formatter = logging.Formatter('%(asctime)s -%(lineno)s- [%(levelname) 5s] - %(message)s')
...
def prepare_chromosome_data(count_data: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    special_chroms = ['chr9', 'chr13', 'chr16', 'chr18', 'chr21', 'chr22', 'chrX', 'chrY']
...
def calculate_zscore_from_normalized(normalized_matrix: np.ndarray, 
                                   mean_file: str, sd_file: str) -> np.ndarray:
    ...
    for i in range(n_samples):
        for j in range(n_samples):
            if sd_data[i, j] != 0:
                zscore_matrix[i, j] = (normalized_matrix[i, j] - mean_data[i, j]) / sd_data[i, j]
            else:
                zscore_matrix[i, j] = 0.0
    
    return zscore_matrix

def create_zscore_dataframe(zscore_matrix: np.ndarray, 
                          index_names: List[str]) -> pd.DataFrame:
...
def calc_zscore_10mb_all(count_file, mean_df, sd_df):
    ...
    # Calculate Z-scores
    z = {j : [(normalized_df.iloc[i,j] - mean_df.iloc[i,j])/sd_df.iloc[i,j] if sd_df.iloc[i,j] != 0.0 else 0.0 for i in range(len(mean_df))] for j in range(len(mean_df.iloc[0]))}
...
def calculate_prizm_zscores(count_file: str, mean_file: str, sd_file: str, 
                           mean_10mb_file: str, sd_10mb_file: str,
                           mean_10mb_all_file: str, sd_10mb_all_file: str) -> PRIZMResult:
    ...
    chr_zscore_matrix = calculate_zscore_from_normalized(chr_normalized, mean_file, sd_file)
    ...
    mb10_zscore_matrix = calculate_zscore_from_normalized(mb10_normalized, mean_10mb_file, sd_10mb_file)
    ...
    zscore_10mb_all_df = calc_zscore_10mb_all(count_file, mean_10mb_all_df, sd_10mb_all_df)
...
def statistical_trisomy_detection(zscore_chr: pd.DataFrame, 
                                target_chromosomes: List[str] = None,
                                confidence_level: float = 0.95,
                                effect_size_threshold: float = 0.8,
                                min_high_scores: int = 15) -> Dict[str, Dict]:
    """
    ...
    for target_chr in target_chromosomes:
        if target_chr not in zscore_chr.index:
            logger.warning(f"Target chromosome {target_chr} not found in Z-score matrix")
            continue
        ...
        analysis_result = analyze_chromosome_distribution(
            target_chr, target_scores, baseline_scores, comparison_chroms,
            confidence_level, effect_size_threshold, min_high_scores
        )
        ...
        if target_chr in ['chr13', 'chr18', 'chr21']:
            ...
            logger.info(f"  - High Z-scores: {analysis_result['high_count']}/{len(target_scores)}")
            logger.info(f"  - Mean Z-score: {analysis_result['mean_zscore']:.3f}")
        
    return results

def analyze_chromosome_distribution(target_chr: str, 
                                  target_scores: np.ndarray, 
                                  baseline_scores: np.ndarray,
                                  comparison_chroms: List[str],
                                  confidence_level: float,
                                  effect_size_threshold: float,
                                  min_high_scores: int) -> Dict:
    """
    ...
    t_stat, p_value = stats.ttest_ind(target_scores, baseline_scores, alternative='greater')
    
    # 2. Effect size (Cohen's d): How large is the difference?
    pooled_std = np.sqrt(((len(target_scores) - 1) * target_std**2 + 
    ...
    baseline_iqr = np.percentile(baseline_scores, 75) - np.percentile(baseline_scores, 25)
    ...
    top_chromosomes = [comparison_chroms[i] for i in target_sorted_indices[:min(10, len(target_sorted_indices))]]
    
    # Decision logic
    decision, confidence = make_statistical_decision(
        p_value, cohens_d, high_count, outlier_count, 
        len(target_scores), confidence_level, effect_size_threshold, min_high_scores
    )
    
    # Detailed results
    result = {
        'decision': decision,
        'confidence': 1 - p_value if p_value < 0.5 else confidence,  # Statistical confidence
        'p_value': p_value,
        ...
        'top_chromosomes': top_chromosomes[:5],  # Top 5 chromosomes with highest Z-scores
        'statistical_summary': {
            't_statistic': t_stat,
...
def make_statistical_decision(p_value: float, 
                            cohens_d: float, 
                            high_count: int, 
                            outlier_count: int,
                            ...
                            confidence_level: float,
                            effect_size_threshold: float,
                            min_high_scores: int) -> Tuple[str, float]:
    """
...
def create_detection_report(results: Dict[str, Dict], output_file: str, sample_name: str):
    """
    Create a detailed statistical detection report
    """
    
    with open(output_file, 'w') as f:
        f.write(f"Statistical Trisomy Detection Report\n")
        ...
        for chrom in key_chroms:
            if chrom in results:
                ...
                f.write(f"{chrom}: {result['decision']} (confidence: {result['confidence']:.1%})\n")
        f.write("\n")
        ...
        for chrom, result in results.items():
            ...
            f.write(f"Effect Size (Cohen's d): {result['effect_size']:.3f} ({result['statistical_summary']['cohens_d_interpretation']})\n")
            f.write(f"Mean Z-score: {result['mean_zscore']:.3f} ± {result['std_zscore']:.3f}\n")
            f.write(f"Baseline Mean: {result['baseline_mean']:.3f} ± {result['baseline_std']:.3f}\n")
            f.write(f"High Z-scores: {result['high_count']}/{result['total_comparisons']} ({result['high_score_ratio']:.1%})\n")
            f.write(f"Outliers: {result['outlier_count']}/{result['total_comparisons']}\n")
...
def create_detection_table(results: Dict[str, Dict], output_file: str, sample_name: str):
    """
    ...
    for chrom in chr_order:
        if chrom in results:
            ...
            row = {
                ...
                'Effect_Size_Interpretation': result['statistical_summary']['cohens_d_interpretation'],
    ...
    # Add metadata header
    with open(output_file, 'w') as f:
        f.write(f"# Statistical Trisomy Detection Results\n")
        f.write(f"# Sample: {sample_name}\n")
        f.write(f"# Analysis Date: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"# \n")
        f.write(f"# Column Descriptions:\n")
        f.write(f"# Decision: Detected/Suspected/Possible/Normal\n")
        f.write(f"# Confidence: Statistical confidence (0-1)\n")
        f.write(f"# P_value: Two-sample t-test p-value\n")
        f.write(f"# Effect_Size_Cohens_d: Cohen's d effect size\n")
        f.write(f"# High_Score_Ratio: Proportion of Z-scores above adaptive threshold\n")
        f.write(f"# Top_5_Chromosomes: Chromosomes with highest Z-scores (separated by |)\n")
        f.write(f"# \n")
...
def run_statistical_trisomy_detection(zscore_chr: pd.DataFrame, 
                                    output_dir: str, 
                                    sample_name: str) -> Dict[str, Dict]:
    """
...
def perform_qc_analysis_old(zscore_df: pd.DataFrame, cutoff: float, output_file: str) -> bool:
...
def plot_zscore_by_chromosome_filtered(df, output_dir, sample_name="sample", extremes_count=2, file_prefix=None):
    ...
    logger.info(f"Most frequent high Z-score chromosomes: {high_value_counts.most_common(3)}")
    logger.info(f"Most frequent low Z-score chromosomes: {low_value_counts.most_common(3)}")
    ...
    for chrom, count in high_value_counts.items():
        if count >= threshold:
            ...
            logger.info(f"Filtering: {chrom} - high values found in {count} chromosomes")
    ...
    # Plot Z-score line
    ax.plot(x_pos, all_z_scores, color='blue', linewidth=1, label='PRIZM Z-score (filtered)')
    
    # Threshold lines
    ax.axhline(y=z_threshold, color='gray', linestyle='--', linewidth=0.8, alpha=0.7, label='Threshold')
    ...
    if above_threshold_x:
        ax.scatter(above_threshold_x, above_threshold_y, color='red', s=15, marker='o', 
                   label='Values above threshold')
    
    if below_threshold_x:
        ax.scatter(below_threshold_x, below_threshold_y, color='darkred', s=15, marker='s', 
    ...
    for i, pos in enumerate(chr_labels):
        if i < len(row_indices):
            ...
            if mapped_name in chromosomes_to_filter:
                ax.text(pos, -5.2, '(filtered)', ha='center', fontsize=8, color='red', style='italic')
    ...
    ax.set_title(f'PRIZM Z-score versus chromosomal position - Sample {sample_name}{filtered_info}', fontsize=14)
    ax.set_xticks([])
    ax.set_yticks([-3, 0, 3])
...
def plot_10mb_zscore_by_chromosome_filtered(df, output_dir, sample_name="10mb", extremes_count=2, file_prefix=None):
    ...
    # 염색체 경계 위치
    x_ticks_loc = [0, 25, 50, 70, 90, 109, 127, 143, 158, 173, 187, 201, 215, 227, 238, 249, 259, 268, 276, 282, 289, 294, 300, 316, 322]
    ...
    indices_to_filter = [idx for idx, count in all_neg_indices.items() if count >= threshold_count]
    ...
    for idx in indices_to_filter:
        if idx < len(filtered_data.index):
            ...
            logger.info(f"Filter : {row_name} (Index {idx}) - {all_neg_indices[idx]} Found below -3")
        ...
        for bin_idx in range(bin_count):
            ...
            for row_idx in filtered_data.index:
                if col_idx < len(filtered_data.columns):
                    try:
                        val = float(filtered_data.loc[row_idx, filtered_data.columns[col_idx]])
                        all_z_scores.append(val)
                    except (ValueError, TypeError):
                        all_z_scores.append(0.0)  # 숫자로 변환할 수 없는 값은 0으로 처리
    ...
    if above_threshold:
        ...
        ax.scatter(above_x, above_y, color='red', s=15, marker='o', label='Values above threshold')

    if below_threshold:
        below_x, below_y = zip(*below_threshold)
        ax.scatter(below_x, below_y, color='darkred', s=15, marker='s', label='Values below threshold')
    ...
    for i, pos in enumerate(chr_labels_pos):
        if i < len(chr_labels):
            # 염색체 이름 표시
            ax.text(pos, -5.8, chr_labels[i], ha='center', fontsize=8, fontweight='bold')
...
def generate_prizm_plots(results: PRIZMResult, output_dir: str, sample_name: str, group: str = ""):
    ...
    try:
        ...
        if results.zscore_10mb_all is not None:
            ...
            valid_ticks_loc = [pos for pos in x_ticks_loc if pos < results.zscore_10mb_all.shape[1]]
            ...
            vline_loc = [x-0.5 for x in valid_ticks_loc if x > 0]  # Exclude the first position
            for vline in vline_loc:
                plt.axvline(x=vline, color='black', linestyle='-', linewidth=1.0, alpha=0.7)
...
def detect_potential_trisomy_for_qc(zscore_matrix, chromosome_names, threshold=2.5):
    ...
    for i, chrom in enumerate(chromosome_names):
        ...
        if row_mean > threshold and col_mean > threshold:
            ...
            logger.info(f"QC: Potential trisomy detected for {chrom} (row_mean: {row_mean:.3f}, col_mean: {col_mean:.3f})")
...
def calculate_diagonal_qc(zscore_matrix, trisomy_candidates=None):
    ...
    if trisomy_candidates:
        filtered_diagonal = [val for i, val in enumerate(diagonal_values) if i not in trisomy_candidates]
        analysis_diagonal = np.array(filtered_diagonal) if filtered_diagonal else diagonal_values
    else:
...
def perform_qc_analysis(zscore_df: pd.DataFrame, cutoff: float, output_file: str) -> bool:
    """
    ...
    trisomy_candidates = detect_potential_trisomy_for_qc(zscore_matrix, chromosome_names)
    ...
    extreme_qc = calculate_extreme_ratio_qc(zscore_matrix, trisomy_candidates=trisomy_candidates)
    variance_qc = calculate_variance_qc(zscore_matrix, trisomy_candidates=trisomy_candidates)
    diagonal_qc = calculate_diagonal_qc(zscore_matrix, trisomy_candidates=trisomy_candidates)
    ...
    # 4. Scoring system
    scores = []
    if extreme_qc['status'] == 'PASS': scores.append(3)
    elif extreme_qc['status'] == 'WARNING': scores.append(2)
    else: scores.append(0)
    
    if variance_qc['status'] == 'PASS': scores.append(3)
    elif variance_qc['status'] == 'WARNING': scores.append(2)
    else: scores.append(0)
    
    if diagonal_qc['status'] == 'PASS': scores.append(3)
    elif diagonal_qc['status'] == 'WARNING': scores.append(2)
    else: scores.append(0)
    ...
    with open(enhanced_report_file, 'w') as f:
        ...
        f.write(f"Legacy QC (75th percentile): {legacy_qc_value:.6f} - {'PASS' if legacy_passed else 'FAIL'}\n")
        
        # Trisomy detection results
        if trisomy_candidates:
            f.write(f"\nTrisomy Detection for QC:\n")
            f.write(f"Potential trisomy chromosomes detected: {len(trisomy_candidates)}\n")
            for idx in trisomy_candidates:
                chrom_name = chromosome_names[idx] if idx < len(chromosome_names) else f"chr{idx+1}"
        ...
        f.write(f"   - Extreme ratio: {extreme_qc['extreme_ratio']:.4f} ({extreme_qc['extreme_count']}/{extreme_qc['total_count']})\n")
        f.write(f"   - Trisomy exclusion: {'Yes' if extreme_qc['excluded_trisomy'] else 'No'}\n")
        f.write(f"   - Threshold: |Z-score| > 3.0\n")
        f.write(f"   - Recommendation: Keep below 5%\n\n")
        ...
        f.write(f"   - Trisomy exclusion: {'Yes' if variance_qc['excluded_trisomy'] else 'No'}\n")
        f.write(f"   - Recommendation: Std should be close to 1.0, mean close to 0\n\n")
        ...
        f.write(f"   - Trisomy exclusion: {'Yes' if diagonal_qc['excluded_trisomy'] else 'No'}\n")
        if diagonal_qc['excluded_trisomy']:
            f.write(f"   - Excluded diagonal count: {diagonal_qc['excluded_count']}\n")
        f.write(f"   - Recommendation: Self-comparisons should be close to 0\n\n")
        ...
        if trisomy_candidates:
            f.write("Trisomy-adjusted thresholds: ≥6 points (PASS), ≥4 points (WARNING), <4 points (FAIL)\n")
        else:
            f.write("Standard thresholds: ≥8 points (PASS), ≥5 points (WARNING), <5 points (FAIL)\n")
        
        f.write(f"\nTrisomy-Aware QC Notes:\n")
        f.write("-" * 25 + "\n")
        f.write("- Chromosomes with consistently high Z-scores are automatically detected\n")
        f.write("- These are excluded from QC calculations to avoid false negatives\n")
        f.write("- Scoring thresholds are adjusted when potential trisomy is detected\n")
        f.write("- This ensures quality samples with true trisomy are not rejected\n")
    
    # Log results
    logger.info(f"Trisomy-Aware QC Analysis:")
    logger.info(f"  - Overall Status: {final_status} (Score: {total_score}/9)")
    logger.info(f"  - Legacy QC: {'PASS' if legacy_passed else 'FAIL'} (75th percentile: {legacy_qc_value:.6f})")
    if trisomy_candidates:
        trisomy_names = [chromosome_names[i] if i < len(chromosome_names) else f"chr{i+1}" for i in trisomy_candidates]
        logger.info(f"  - Potential Trisomy: {', '.join(trisomy_names)} (excluded from QC)")
    logger.info(f"  - Extreme Values: {extreme_qc['status']} (ratio: {extreme_qc['extreme_ratio']:.4f})")
    logger.info(f"  - Variance: {variance_qc['status']} (std: {variance_qc['std_deviation']:.4f})")
    logger.info(f"  - Diagonal: {diagonal_qc['status']} (std: {diagonal_qc['diagonal_std']:.4f})")
...
def identify_potential_trisomies(zscore_df, threshold=3.0):
    ...
    for i, chrom in enumerate(zscore_df.index):
        ...
        if len(high_scores) > len(zscore_df.columns) * 0.7:  # If >70% of comparisons are high
...
def create_prizm_summary_report(sample_name, results, output_dir, trisomy_results=None):
    ...
    try:
        ...
        with open(report_file, 'w') as f:
            f.write(f"PRIZM Analysis Summary Report\n")
            f.write(f"Sample: {sample_name}\n")
            f.write(f"Analysis Date: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write("="*50 + "\n\n")
            ...
            f.write(f"10mb_all Z-scores Matrix Shape: {results.zscore_10mb_all.shape}\n")
...
def run_multiple_prizm_analysis(sample_name, gender, labcode, config, analysis_dir, data_dir):
    """
    ...
    logger.info(f"=== Starting comprehensive PRIZM analysis for {sample_name} (Gender: {gender}) ===")
    ...
    for analysis in analyses:
        ...
        if missing_refs:
            logger.warning(f"Missing reference files for PRIZM {analysis_type}: {missing_refs}")
            logger.warning(f"Skipping PRIZM {analysis_type} analysis")
            all_success = False
            continue
        ...
        try:
            ...
            if results:
                ...
                save_zscore_results(results.zscore_chr, f"{analysis_output_dir}/{sample_name}_{analysis_type}.prizm.zscore.txt")
                save_zscore_results(results.zscore_10mb, f"{analysis_output_dir}/{sample_name}_{analysis_type}.prizm.zscore.10mb.txt")
                
                if enable_10mb_all and results.zscore_10mb_all is not None:
                    save_zscore_results(results.zscore_10mb_all, f"{analysis_output_dir}/{sample_name}_{analysis_type}.prizm.zscore.10mb_all.txt")

                logger.info(f"PRIZM {analysis_type} analysis completed successfully")
                logger.info(f"  - Chromosome Z-scores matrix: {results.zscore_chr.shape}")
                logger.info(f"  - 10mb Z-scores matrix: {results.zscore_10mb.shape}")
                if enable_10mb_all and results.zscore_10mb_all is not None:
                    logger.info(f"  - 10mb_all Z-scores matrix: {results.zscore_10mb_all.shape}")
                ...
                if enable_trisomy_detection:
                    try:
                        ...
                        logger.info(f"Statistical trisomy detection completed for {analysis_type}")
                    except Exception as e:
                        logger.error(f"Error in statistical trisomy detection for {analysis_type}: {e}")
                
                # Create summary report for this analysis
                create_prizm_summary_report(f"{sample_name}_{analysis_type}", results, analysis_output_dir, trisomy_results)
            else:
...
def main():
    ...
    input_group.add_argument('-c10', dest='count_file_10mb', required=True,
                            type=lambda x: file_exists(parser, x),
                            help='10mb count file')
    input_group.add_argument('-m', dest='mean', required=True,
                            type=lambda x: file_exists(parser, x),
                            help='Reference mean file')
    input_group.add_argument('-s', dest='sd', required=True,
                            type=lambda x: file_exists(parser, x),
                            help='Reference standard deviation file')
    input_group.add_argument('-m10', dest='mean_10mb', required=True,
                            type=lambda x: file_exists(parser, x),
                            help='10mb reference mean file')
    input_group.add_argument('-s10', dest='sd_10mb', required=True,
                            type=lambda x: file_exists(parser, x),
                            help='10mb reference standard deviation file')
    input_group.add_argument('-m10all', dest='mean_10mb_all',
                            type=lambda x: file_exists(parser, x),
                            help='10mb_all reference mean file')
    input_group.add_argument('-s10all', dest='sd_10mb_all',
                            type=lambda x: file_exists(parser, x),
                            help='10mb_all reference standard deviation file')
    input_group.add_argument('-q', dest='qc_cutoff', type=float, default=3.0,
                            help='QC cutoff threshold')
    input_group.add_argument('-single', dest='single_output', default='N',
                            choices=['Y', 'N'], help='Single output mode')
    input_group.add_argument('--sample_name', dest='sample_name', 
                            help='Sample name for plot titles')
    input_group.add_argument('--skip_plots', dest='skip_plots', action='store_true',
                            help='Skip plot generation')
    input_group.add_argument('--enable_10mb_all', dest='enable_10mb_all', action='store_true',
                            help='Enable 10mb_all Z-score calculation')
    input_group.add_argument('--enable_trisomy_detection', dest='enable_trisomy_detection', action='store_true',
                            help='Enable trisomy detection')
    
    # Options
    options_group = parser.add_argument_group("Options")
    options_group.add_argument('--debug', action='store_true', default=False,
                              help='Enable debug mode')
    options_group.add_argument('-v', '--version', action='version', 
    ...
    config = PRIZMConfig(
        qc_cutoff=args.qc_cutoff,
        single_output=(args.single_output == 'Y')
    )
    ...
    try:
        ...
        if args.enable_10mb_all and args.mean_10mb_all and args.sd_10mb_all:
            ...
            chr_normalized = calculate_normalized_matrix(chr_summary)
            chr_zscore_matrix = calculate_zscore_from_normalized(chr_normalized, args.mean, args.sd)
            
            chr_index_dict = {i: f'chr{i+1}' for i in range(22)}
            chr_index_dict.update({22: 'chrX', 23: 'chrY'})
            
            chr_zscore_df = create_zscore_dataframe(chr_zscore_matrix, list(chr_index_dict.values()))
            
            # Calculate 10mb-level Z-scores
            mb10_normalized = calculate_normalized_matrix(mb10_data)
            mb10_zscore_matrix = calculate_zscore_from_normalized(mb10_normalized, args.mean_10mb, args.sd_10mb)
            
            mb10_index_dict = {i: mb10_data.iloc[i]['chr'] for i in range(len(mb10_data))}
            mb10_zscore_df = create_zscore_dataframe(mb10_zscore_matrix, list(mb10_index_dict.values()))
        ...
        output_dir = "" if config.single_output else '/'.join(args.count_file_10mb.split('/')[:-1])
        qc_file = os.path.join(output_dir, f"{filename}.prizm.qc.txt") if output_dir else f"{filename}.prizm.qc.txt"
...
def run_prizm_analysis(count_file_10mb: str, mean_file: str, sd_file: str, 
                       mean_10mb_file: str, sd_10mb_file: str, 
