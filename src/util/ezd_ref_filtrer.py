#!/usr/bin/env python3
"""
염색체 파일에서 임계값 기반 샘플 필터링 스크립트 (최종 수정 버전)
- orig, fetus, mom 디렉토리의 chr1.txt~chr22.txt 파일 처리
- N 타입 샘플: UR >= UR_min OR Z >= Z_min 인 샘플 제거
- non-N 타입 샘플: UR < UR_min OR Z < Z_min 인 샘플 제거
"""
...
def filter_chr_samples_by_threshold(chr_dir, threshold_file):
    ...
    새로운 필터링 정책:
    - N 타입 샘플: UR < UR_min AND Z < Z_min 조건만 유지 (고품질 Normal만)
    - non-N 타입 샘플: UR >= UR_min AND Z >= Z_min 조건만 유지 (고품질 Positive만)
    ...
    try:
        ...
        print(f"필터링 정책:")
        print(f"  - N 타입: UR < UR_min AND Z < Z_min 조건만 유지")
        print(f"  - non-N 타입: UR >= UR_min AND Z >= Z_min 조건만 유지")
        ...
        for i in range(1, 23):
            ...
            try:
                ...
                # 필터링 적용: 
                # 1. N 타입 샘플: UR < UR_min AND Z < Z_min 조건만 유지 (고품질 Normal만)
                # 2. non-N 타입 샘플: UR >= UR_min AND Z >= Z_min 조건만 유지 (고품질 Positive만)
                
                # N 타입 샘플 (UR < UR_min AND Z < Z_min 조건만 유지)
                n_samples = valid_df[valid_df['type'] == 'N']
                filtered_n = n_samples[(n_samples['UR_numeric'] < ur_min) & (n_samples['Z_numeric'] < z_min)]
                
                # N이 아닌 타입 샘플 (UR >= UR_min AND Z >= Z_min 조건만 유지)
                ...
                # 필터링된 N 샘플 + 필터링된 non-N 샘플 결합
                filtered_df = pd.concat([filtered_n, filtered_non_n], ignore_index=True)
                ...
                print(f"    샘플 처리:")
                print(f"      N 타입: {len(n_samples)} → {len(filtered_n)}개 (UR < {ur_min} AND Z < {z_min} 조건)")
                print(f"      non-N 타입: {len(non_n_samples)} → {len(filtered_non_n)}개 (UR >= {ur_min} AND Z >= {z_min} 조건)")
                print(f"      전체: {original_count} → {filtered_count} ({removed_count}개 제거)")
                
                # 제거된 샘플들 정보 출력 (처음 6개만)
                if removed_count > 0:
                    # N 타입에서 제거된 샘플들 (UR >= UR_min OR Z >= Z_min)
                    removed_n = n_samples[~((n_samples['UR_numeric'] < ur_min) & (n_samples['Z_numeric'] < z_min))]
                    # non-N 타입에서 제거된 샘플들 (UR < UR_min OR Z < Z_min)
                    removed_non_n = non_n_samples[~((non_n_samples['UR_numeric'] >= ur_min) & (non_n_samples['Z_numeric'] >= z_min))]
                    
                    if len(removed_n) > 0:
                        print(f"    제거된 N 타입 샘플 (UR >= {ur_min} OR Z >= {z_min}):")
                        for idx, row in removed_n.head(3).iterrows():
                            ur_val = row.get('UR', 'N/A')
                            z_val = row.get('Z', 'N/A')
                            sample_name = row.get('sample', 'Unknown')
                            print(f"      {sample_name} (N) - UR:{ur_val}, Z:{z_val}")
                    
                    if len(removed_non_n) > 0:
                        print(f"    제거된 non-N 타입 샘플 (UR < {ur_min} OR Z < {z_min}):")
                        for idx, row in removed_non_n.head(3).iterrows():
                            ur_val = row.get('UR', 'N/A')
                            z_val = row.get('Z', 'N/A')
                            sample_name = row.get('sample', 'Unknown')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='염색체 파일 임계값 기반 필터링 (최종 수정 버전)')
...
if __name__ == "__main__":
    # 사용 예시
    if len(sys.argv) == 1:
        print("염색체 파일 임계값 기반 필터링 스크립트 (최종 수정 버전)")
        print("\n새로운 필터링 정책:")
        print("  - N 타입 샘플: UR < UR_min AND Z < Z_min 조건만 유지 (고품질 Normal만)")
        print("  - non-N 타입 샘플: UR >= UR_min AND Z >= Z_min 조건만 유지 (고품질 Positive만)")
        print("\n사용법:")
