...
# ==============================================================
def draw_heatmap(zscore, index_dict, color_list, ax):
    #ax = plt.gca()
...
def run_algorithm(count_list):
    ...
    for count_file in count_list:
        ...
        try:
            ...
            if gender_zipped_list[1][gender_zipped_list[0].index(filename_gendersearch)] == 'XX':
                logger.debug("%s --> XX" %(filename_gendersearch))
                command = "python " + CFG_DecisionAlgorithm + " -c10 " + count_file + " -m " + CFG_FemaleMean + " -s " + CFG_FemaleSd + " -m10 " + CFG_Female10mbMean + " -s10 " + CFG_Female10mbSd + " -lite Y -single N -q 3.0"
...
# ==============================================================
def runParallel(func, count_list):
    proc = []
...
if __name__ == '__main__':
    ...
    ogroup.add_argument('-target', dest='target', metavar='Y|N',default='N', help='calc zscore & save heatmap')
    ...
    if args.reference == 'Y':
        ...
        male_mean_df, male_sd_df, male_10mb_mean_df, male_10mb_sd_df = makeReference_10mb(file_list, CFG_MaleMean, CFG_MaleSd, CFG_Male10mbMean, CFG_Male10mbSd, autosomal=False)

        logger.debug(male_mean_df.head())
        logger.debug(male_sd_df.head())
        logger.debug(male_10mb_mean_df.head())
        logger.debug(male_10mb_mean_df.head())
        ...
        with open(CFG_Male10mbMean, 'w') as f:
            male_10mb_mean_df.to_csv(f, sep='\t', index=False, header=False)
        with open(CFG_Male10mbSd, 'w') as f:
            male_10mb_sd_df.to_csv(f, sep='\t', index=False, header=False)
        ...
        logger.debug(male_mean_df.head())
        logger.debug("Male Mean DF : %s" % str(male_mean_df.shape))
        logger.debug(female_sd_df.head())
        logger.debug("Female Sd DF : %s" % str(female_sd_df.shape))
        logger.debug(male_10mb_mean_df.head())
        logger.debug("Male 10mb Mean DF : %s" % str(male_10mb_mean_df.shape))
        logger.debug(female_10mb_mean_df.head())
    ...
    # -------------------------------------------------------
    # 6. calculate z-score & draw heatmap
    # -------------------------------------------------------
    if args.target == 'Y':
        logger.info("Generate z-score for targetted directory %s" %(CFG_SampleRunDir))
        ...
        with open(CFG_GenderList) as f:
            ...
            runParallel(run_zscore, list(split_list))
    ...
    if args.decision == 'Y':
        ...
        with open(CFG_GenderList) as f:
            ...
            split_list = split_seq_by_num_of_sublist(count_list, args.core)
            runParallel(run_algorithm, list(split_list))
    ...
    if args.decision_ng == 'Y':
        ...
        split_list = split_seq_by_num_of_sublist(count_list, args.core)
        runParallel(run_algorithm_noGender, list(split_list))
